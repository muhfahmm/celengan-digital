...
<body>
    <div class="container">
        ...
        <script>
            ...
            const rawColors = <?= json_encode($colors); ?>; // masih dipakai kalau mau
            ...
            function colorForDiff(diff) {
                if (diff > 0) return '#41A67E'; // hijau (naik)
                if (diff < 0) return '#BF1A1A'; // merah (turun)
                return 'rgba(180,180,180,0.6)'; // netral (nol)
            }

            // ==== MERGE TRANSAKSI PER TANGGAL (memilih warna berdasarkan totalDiff) ====
            function mergeByDate(labels, awal, akhir, colors) {
                const map = Object.create(null); // objek bersih

                // labels di sini diasumsikan sudah berurut kronologis
                ...
                // Object.keys akan mengembalikan insertion order sesuai traversal labels
            ...
            function filterData(range) {
                ...
                if (range === '1D' || range === '1W') {
                    // Tidak merge: gunakan warna asli tiap transaksi
                    data = {
                        labels,
                        awal,
                        akhir,
                    ...
                    // Merge termasuk ALL: warna ditentukan oleh total diff per tanggal
            ...
            // ==== RUN ====
            initChart();
            filterData('ALL'); // tampil awal sebagai ALL (sudah di-merge & warna per net diff)
            autoRefreshDaily();

            // set tombol active dan event listener
